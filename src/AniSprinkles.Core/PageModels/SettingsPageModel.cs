...
public partial class SettingsPageModel : ObservableObject
{
    ...
    // Held for 20 seconds rather than the default: a settings save that failed is only recoverable
    // if the user actually notices the Retry.
    private Task ShowSaveFailureSnackbarAsync(string message)
        => _feedback.ShowSnackbarAsync(message, "Retry", () => _ = SaveSettingsAsync(), TimeSpan.FromSeconds(20));
