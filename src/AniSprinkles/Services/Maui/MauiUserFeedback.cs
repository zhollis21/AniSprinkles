...
public sealed class MauiUserFeedback(ILogger<MauiUserFeedback> logger) : IUserFeedback
{
    ...
    public async Task ShowSnackbarAsync(string message, string actionText, Action action, TimeSpan? duration = null)
    {
        try
        {
            await Snackbar.Make(message, action: action, actionButtonText: actionText, duration: duration ?? TimeSpan.FromSeconds(5))
