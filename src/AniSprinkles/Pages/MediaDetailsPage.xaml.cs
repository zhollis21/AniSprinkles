...
public partial class MediaDetailsPage : ContentPage, IQueryAttributable
{
    ...
    protected override void OnNavigatedTo(NavigatedToEventArgs args)
    {
        ...
        _loader.OnAppearing();
        Logger.LogInformation(
            "NAVTRACE {TraceId} MediaDetailsPage.OnNavigatedTo at {NowUtc:O} (+{SinceTapMs}ms)",
            _activeNavTraceId,
            DateTimeOffset.UtcNow,
            NavigationTelemetryHelper.GetElapsedFromTapMilliseconds(_activeNavStartUtc));
        ScheduleLoad();
    ...
    protected override void OnNavigatedFrom(NavigatedFromEventArgs args)
    {
        ...
        // away. List ops recreate the scope via EnsurePageScope, so the sort popup — which fires this
        // too — stays harmless.
