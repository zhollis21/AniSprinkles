...
- **Unit tests** — `tests/AniSprinkles.UnitTests/` (xUnit, `net10.0`, NSubstitute). Project-references `src/AniSprinkles.Core/`; there are no link-compile globs any more (#62). Covers pure algorithms, PageModel state machines, and repo-level file validation (`.xaml` well-formedness, icon-glyph resolution). Test doubles live in `Fakes/`. `tests/AniSprinkles.UITests/` is still a scaffold for future device tests.
  ...
  - **Known gap**: `AppSettings.Load/Save/Clear` still use the static `Preferences.Default`, so any path ending in `AppSettings.SyncFromViewer` (e.g. a successful `SettingsPageModel.LoadAsync`) is not testable. Giving `AppSettings` a preferences seam is open in #52.
  - Test classes that write to the process-wide `AppSettings` statics must join the `AppSettings` collection (`AppSettingsCollection.Name`) so they do not run in parallel.
...
- `CIAniListClient` (`src/AniSprinkles/Services/CI/`) — returns hardcoded anime list, user profile, and details-page fixtures
